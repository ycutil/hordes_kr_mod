...
## 스케줄
- **클라이언트에 스케줄 없음**(서버 구동). 다음 윈도우 시각을 클라에서 못 읽음. **확정: ~3h 간격(KST 3/6/9/12/15/18/21/0), 윈도우 길이 ~35분.** 활성 여부는 채팅 GM 메시지/`serverWarUpdate`로 판단.

## 라이브 관찰 (2026-06-01 KST 0:00 윈도우, 활성 중)
- 채팅 GM: "An Obelisk has appeared!" → "Obelisk captured by **{Guild}**!" 사이클 **~2-4분 주기**. 점령 주체 = **길드**(Bloodlust, Vanguard 등) = **조직 PvP**.
- **"The Obelisk will judge the battlefield in 20 seconds!"** = 점령 유지 시 전장 심판(광역효과) 발동 메커니즘.
- 오벨리스크는 **오픈월드 목표**(별도 인스턴스 아닌 듯) — 월드 특정 위치에 스폰, 근처서 점령 다툼. 채팅에 좌표 없음.
- **현실적 한계**: 솔로 AI봇이 (리콜/위치 모르고) 활성 PvP존까지 가서 길드 zerg 상대로 생존·점령 기여하는 건 비현실적. 봇 전투코드(적진영 타겟팅+검증된 farmStep)는 **오벨존에 배치되면** 싸울 수 있으나, **자율 진입+생존은 불가** → 기어 갖춘 캐릭을 사용자가 파일럿하거나 코디된 그룹 합류 필요. (테스트 캐릭 Mharu는 리콜 미보유라 이동조차 막힘.)
